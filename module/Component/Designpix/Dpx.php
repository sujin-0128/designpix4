...
class Dpx
{
	...
	public function getDiscountBundleGroupList()
	{
		...
		$strSQL = ' SELECT COUNT(*) AS cnt FROM es_discount_bundle_group ' ;
		...
		$this->db->strField = "
			...
			bg.groupCd,
			bg.groupNm,
			bg.allowNoBundleSale,
			bg.showNoBundlePopup,
			bg.preCartBundlePopup,
			bg.regDt,
			bg.modDt,
			(
				SELECT COUNT(*) FROM es_discount_bundle_group_goods bgg
				WHERE bgg.groupCd = bg.groupCd
			) AS goodsCount
		";
		$this->db->strFrom = 'es_discount_bundle_group bg';
		...
		// 검색 카운트
        $strSQL = ' SELECT COUNT(*) AS cnt FROM es_discount_bundle_group  as bg ';
        ...
        $strSQL = 'SELECT ' . array_shift($query) . ' FROM es_discount_bundle_group as bg ' . implode(' ', $query);
			...
			switch ($this->search['key']) {
				...
				case 'goodsNo':
					$this->arrWhere[] = 'EXISTS (
						SELECT 1 FROM es_discount_bundle_group_goods bgg
	...
	public function getInfoDiscountBundleGroup($sno = null, $goodsField = null, $arrBind = null, $dataArray = false)
	{
		...
		$query = $this->db->query_complete();
        $strSQL = 'SELECT ' . array_shift($query) . ' FROM es_discount_bundle_group dbg ' . implode(' ', $query);
	...
	public function getInfoDiscountBundleGroupGoods($groupCd,$arrBind = null)
	{
        ...
        $join[] =  ' INNER JOIN es_discount_bundle_group as dbg ON dbg.groupCd = dbgg.groupCd ';
        ...
        $query = $this->db->query_complete();
        $strSQL = 'SELECT ' . array_shift($query) . ' FROM es_discount_bundle_group_goods as dbgg ' . implode(' ', $query);
	...
	public function saveInfoDiscountBundleGroup($arrData)
	{
        ...
        if ($arrData['mode'] == 'group_modify') {
            ...
            $this->db->set_update_db('es_discount_bundle_group', $arrBind['param'], 'sno = ?', $arrBind['bind']);


        } else {
            $arrData['groupCd'] = $this->newGroupCode();
            $arrBind = $this->db->get_binding(DBTableField::tableDiscountBundleGroup(), $arrData, 'insert');
            $this->db->set_insert_db('es_discount_bundle_group', $arrBind['param'], $arrBind['bind'], 'y');
        ...
        if ($arrData['addGoodsNoData']) {
            ...
            //관련 상품 새로 지우고 새로 등록
            $this->db->set_delete_db('es_discount_bundle_group_goods', 'groupCd = "' . $arrData['groupCd'] . '"');
            ...
            foreach ($arrData['addGoodsNoData'] as $k => $v) {
                ...
                $this->db->set_insert_db('es_discount_bundle_group_goods', $arrBind['param'], $arrBind['bind'], 'y');
            }
        }
    ...
    {
        $strSQL = 'SELECT MAX(substring(groupCd,2)) FROM es_discount_bundle_group ';
	 ...
	 * @sjlee
     */
	public function checkAllowNoBundleSale()
	{
		$getValue = Request::get()->toArray();
		...
		if ($goodsNo) {
				...
				$qry = "SELECT dbg.*, dbgg.*
						FROM es_discount_bundle_group_goods as dbgg
							JOIN es_discount_bundle_group as dbg ON dbgg.groupCd = dbg.groupCd
						WHERE dbgg.goodsNo = ?
						limit 1
					";
			$arrBind = [];
	...
	public function updateBundleType($arrData)
	{
		...
		foreach ($arrData['itemGoodsNo'] as $k => $v) {
			$qry = "UPDATE es_discount_bundle_group_goods set bundleType = '".$arrData['bundleType']."' where groupCd = '" . $arrData['groupCd'] . "' and goodsNo = ".$v." ";
	...
	public function deleteDiscountBundleGroup($sno, $groupCd)
	{
        ...
        $this->db->set_delete_db('es_discount_bundle_group', 'sno = ?', $arrBind['bind']);

        unset($this->arrBind);

        $this->db->set_delete_db('es_discount_bundle_group_goods', 'groupCd = "' . $groupCd . '"');
	}
